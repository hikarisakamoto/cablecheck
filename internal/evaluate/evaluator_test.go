...
func TestClassifyFoldOrdering(t *testing.T) {
	...
	t.Run("large negotiated speed reduction yields poor", func(t *testing.T) {
		f := cleanFacts()
		...
		res := Evaluate(f)
		if res.Class != model.HealthPoor {
			t.Errorf("class = %v, want POOR (PHY-06 severe reduced speed)", res.Class)
		}
	})
}
...
func TestEvaluateDeterministic(t *testing.T) {
	...
	if first.RulesVersion != "1.10.0" {
		t.Errorf("RulesVersion = %q, want 1.10.0", first.RulesVersion)
	}
	if first.Score == nil {
