...
CableCheck protocol version `"1"` is a framed-JSON protocol over plain TCP. PC1 is the listener/coordinator and PC2 is the dialing worker. Version 1 has no negotiation: envelope versions must match exactly.

This document describes the implemented wire contract in `internal/protocol` and its lifecycle in `internal/peer`.
...
The length excludes the four-byte prefix. `io.ReadFull` reads both prefix and body under one absolute per-frame deadline.
...
A declared length outside 2 bytes–1 MiB or invalid envelope JSON is fatal. The reader does not try to skip or resynchronize: a partial header/body read leaves stream position unknowable. After any read error, `protocol.Conn` is poisoned and later reads fail immediately. Oversized outbound envelopes are rejected rather than truncated.

Writes marshal the complete envelope, prepend the length, and issue one connection write while holding a mutex. At the session level, message-ID allocation and writing are also one locked operation, preserving wire order across the event loop and heartbeater.
...
- `testId` is empty before `hello_ack`; PC1 assigns an ID such as `ct-20260719-143205-a1b2c3d4`. Post-handshake frames normally carry it. The current verifier tolerates an empty value, but logs and drops a non-empty foreign value.
- `messageId` is unique per sender and monotonically increasing: `pc1-00000001`, `pc2-00000001`, and so on.
- `inReplyTo` correlates an acknowledgement, progress message, result, or completion with the triggering message when the sender has such a message ID.
- `timestamp` is the sender's UTC time and marshals as RFC 3339 with fractional precision as needed. Countdown synchronization does not trust it.
- `payload` is decoded according to `type` into one of the fixed structs below.

Unknown JSON fields are tolerated for forward compatibility. Payloads are not decoded into arbitrary Go types. A duplicate detector remembers 128 recent IDs and the maximum sequence per role. During the strict handshake, a duplicate or non-increasing ID is a retryable protocol failure; after handshake it is logged and dropped.
...
`iperf3.json` is the one required peer capability. Other flags select fallbacks or optional behavior.
...
- `step` and `totalSteps`: schema fields for 1-based display metadata. The
  current coordinator RPC adapter leaves both at their zero values; the
  session's plan progress is driven locally.
...
`TestProgress` contains `stage`, `percent` (`-1` means indeterminate), `text`, and an optional string-to-number `metrics` map. Progress is correlated through envelope `inReplyTo` and is throttled to at most one update per second.
...
cable-window end result is `{"selfInflictedCarrierEvents":N}` and is handled
by the peer layer. A second non-cancel request while an operation is active
receives `rejected` immediately.
...
current adapter leaves their envelope `inReplyTo` empty.
...
PC1 listens on exactly `--local-ip:--control-port`; it never binds a wildcard address. PC2 binds the connection's source to its own `--local-ip`, uses a 5-second attempt timeout, retries every 2 seconds, and gives up after 60 seconds total.
...
PC1 validates PC2's claimed `peerIp` against PC1's local address and its `localIp` against PC1's configured peer. It compares the token after hashing both inputs with SHA-256 and using `subtle.ConstantTimeCompare`. PC1 assigns the test ID from a UTC timestamp plus four random bytes. A foreign `testId` in the handshake capabilities exchange is a protocol error; after handshake, a frame with a non-empty foreign ID is logged and dropped.

### Handshake failures

The coordinator and worker map failed establishment differently: PC1 treats a connecting side that cannot complete a valid handshake as peer/orchestration exit 5; PC2 treats its own connection/handshake failure as configuration/dependency exit 4.
...
The `cablecheckVersion` values are recorded but are not required to match;
compatibility is decided by the exact control-protocol version and required
capabilities.
...
Under `--non-interactive`, each side sends `ready` automatically. A peer's early `ready` is remembered while the local side still waits; it does not bypass local confirmation.

When PC1 knows both sides are ready, it sends `start_confirmation` with `startInMs:3500`. PC1 starts its own countdown from sending; PC2 starts from frame arrival. Each prints `3… 2… 1… GO` and transitions from `ready` to `testing` at the end. `startAt` is informational only because the hosts' clocks may differ.
...
`inReplyTo` correlates progress/results with the request ID. The worker applies `timeoutMs` through an operation context. PC1 waits the same timeout plus 10 seconds of call grace. If no result arrives, even while heartbeats continue, PC1 emits `abort(request_timeout)` and ends the session. Late results without a pending call are logged and dropped.

Both sides tick every 5 seconds, but send a heartbeat only when no successful frame write occurred for at least 80% of that interval—4 seconds at defaults. Ordinary traffic therefore suppresses redundant heartbeats. A heartbeat carries the sender's state and active operation.

`ReadEnvelope` gets a fresh 20-second deadline for every inbound frame. Timeout, EOF, reset, malformed frame, or truncation is terminal; there is no post-handshake reconnect in protocol v1. A connection loss becomes a `peer_lost` session outcome and exit 5, with PC1 preserving partial measurements when possible.
...
Transfer runs PC1 → PC2 during `generating_report` only when PC1 has not disabled it and PC2 advertised `acceptReportTransfer:true`.
...
3. PC1 streams one file at a time with contiguous zero-based `seq` values and
   byte offsets. PC2 recognizes `seq:0, offset:0` as the start or restart of
   an attempt and enforces the expected name, exact running offset, chunk cap,
   and declared file-size boundary.
4. PC2 writes `<name>.part`, hashes incrementally, and on `last:true` compares both final byte count and SHA-256. A match is fsynced, closed, renamed, and acknowledged `ok:true`.
5. An offset or digest failure deletes the partial file and sends `ok:false`. PC1 retries that entire file once. A second rejection ends the transfer callback; the session emits `warning{code:"report_transfer_failed"}` and continues to `complete`.
6. Unexpected names, oversize data, local filesystem failure, or other non-retryable faults decline the manifest so the sender stops cleanly. No unverified partial file is retained.
7. PC1 sends `complete`; PC2 answers with its own `complete`, and both enter `completed`.

Acknowledgements are per file, not per chunk. `raw/` artifacts are never part of the manifest. A transfer failure is advisory and never changes the health verdict or its exit code.

## Cable-test window coordination

The opt-in ethtool cable diagnostic can drop the same link carrying the control channel. CableCheck appends it after the normal plan so ordinary before/after counters exclude the deliberately disruptive window.
...
Self-inflicted monitoring events carry `selfInflicted:true`; their carrier counts are subtracted from physical carrier evidence. Failure to recover the peer after a locally completed diagnostic is best-effort: the local cable-test result is retained with a limitation note.
...
- The token is plaintext on the wire. For comparison, both strings are first SHA-256 hashed and the fixed-size digests are compared with `subtle.ConstantTimeCompare`, avoiding content and length-dependent comparison behavior.
- Tokens and raw hello payloads are not logged, and no report model field can contain the token.
- PC1 binds only the configured local IP and accepts only the configured peer source IP.
- Frames are size-capped before allocation; report names and bytes are separately constrained.
- Envelope types, payload structs, operation names, parameter decoders, and result decoders are fixed. There is no reflection-based arbitrary deserialization and no remote shell command string.
- Unknown JSON fields and unknown message types are tolerated for forward compatibility, but they do not grant new operations.

Authentication is not confidentiality. Anyone able to observe the trusted link can read the token and results, so do not expose the control port on an untrusted network.
