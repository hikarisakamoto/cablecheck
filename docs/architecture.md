...
The diagram is conceptual, not a complete import graph, but its constraints are concrete:
...
- `reporting` may import only `model`, and a dependency-purity test enforces this. That way offline regeneration can't accidentally rerun tests or evaluation.
...
- `app` is the composition root. It's the layer allowed to know configuration, networking, test plans, evaluation, reporting, logging, and exit policy.

## Run lifecycle

`cli` resolves a `config.RunConfig` and creates `app.App`. On PC1, `App.Start` binds the control listener before returning. The background run then runs local preflight, creates a report directory, attaches logging, builds the mode-specific suite, and enters `peer.Run`.
...
After the synchronous handshake, one event loop owns all session state: readiness flags, countdown, peer heartbeat state, invalid-state strikes, transfer routing, and the terminal finish specification. Producers never mutate those fields. They send typed values through a 64-entry `events` channel instead:
...
Every producer send selects on the session context, so a producer can't stay blocked after teardown. Network reads are the one exception to context cancellation: a blocked `net.Conn` read is released by closing the connection.

Inbound envelopes first pass message-ID duplicate/order checks, exact protocol-version checks, and assigned-test-ID checks. The loop then dispatches by message type. State-inappropriate frames get a `warning{code:"invalid_state"}`, and three such frames abort the session. Unknown message types and late unmatched results are logged and dropped.
...
The event loop performs ordinary response writes. The other allowed initiators
are the heartbeater, the plan driver's synchronous RPC calls, the report
callback's narrow `ReportChannel`, and the bounded farewell helper. None of them
write the TCP connection directly; all converge on the same serialized
`mintAndWrite` path. Executor progress and results are routed back through the
...
A few short-lived helpers exist around this core. PC1 uses a listener watcher during blocking `Accept`. A farewell-abort helper is bounded by two seconds. Soak mode uses a budget watcher inside the plan. And `internal/app` owns a joinable sysfs monitor during `testing`.

The session teardown order matters:
...
Waiting before closing the connection would deadlock a reader blocked in `ReadEnvelope`, so shutdown and leak tests cover this order.
...
A nonzero command exit is data, not automatically an infrastructure error. A `ping` exit 1, for example, means loss. Parsers decide how to interpret the result, and raw output is preserved independently of parsed model values.
...
- `clock/clocktest.FakeClock` advances only when the test asks it to. `BlockUntilWaiters` prevents timer-registration races before `Advance`.
...
- Parser tests consume committed byte fixtures and do no I/O beyond reading testdata.
- Peer unit tests use `net.Pipe` or injected transports, scripted stdin, fake clocks, and state hooks.
- Runner process-lifecycle tests re-execute the test binary as a helper process, so they don't depend on installed network tools.
...
The manual `scripts/demo-e2e.sh` test is kept separate on purpose. It uses loopback plus repository stub tools to exercise the built binary. Loopback is virtual, so its expected health result is `INCONCLUSIVE`.
