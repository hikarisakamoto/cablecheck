...
| PHY-06 reduced-speed | NegotiatedSpeed < ExpectedSpeed (both known): ≤50% of expected → POOR; otherwise WARNING. Conservative text lists cable/pairs/NIC/config causes. | WARNING/POOR |
...
Expected outcomes baked into the seeds: healthy → EXCELLENT/exit 0; reduced-speed → POOR (PHY-06 + PERF-01); crc-errors → POOR or FAILED (PHY-02 band); host-limited → INCONCLUSIVE (PERF-01 + HOST-01, clean counters); failed → FAILED (PHY-01 + PHY-03).
