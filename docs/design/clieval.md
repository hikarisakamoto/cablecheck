...
Read the file (size cap 64 MiB), `json.Unmarshal` into `model.Report`, validate `schemaVersion` major == 1 (else ExitConfig with a message), and re-render report.md + summary.txt next to the JSON (or `--output`). **No re-evaluation**: Evaluation is part of the record. The purity of `RenderMarkdown`/`RenderSummary` (only `*model.Report` in, bytes out) is what makes this correct, and the dependency test from §0 enforces it structurally.
...
- Each scenario (`healthy`, `reduced-speed`, `crc-errors`, `host-limited`, `failed`) is a seed: a pre-Evaluation `model.Report` built in Go code from the same parsed-result structs the parsers produce. Values are traceable to `testdata/` fixtures; the crc-errors seed, for example, uses the deltas from the Realtek CRC fixture. The generator runs the *real* pipeline slice in scope: `evaluate.FactsFromReport` → `evaluate.Evaluate` → set `Evaluation` → `reporting.RenderJSON/RenderMarkdown/RenderSummary` → write `examples/<name>/{report.json,report.md,summary.txt}`.
