...
Scope: `internal/cli`, `internal/config`, `internal/model`, `internal/evaluate`, `internal/reporting`, `internal/app`, `internal/logging`, `cmd/cablecheck`. Stdlib only. All decisions below are final for this scope.
...
`Run` dispatches on `args[0]` ∈ {`run`, `doctor`, `report`, `version`}; no args or `help`/`-h`/`--help` prints top-level usage (subcommand list + one-line descriptions) and returns 0 for explicit help, 4 for missing/unknown subcommand. Each subcommand builds its own `flag.NewFlagSet(name, flag.ContinueOnError)` with `fs.SetOutput(stderr)` and a custom `fs.Usage` (grouped, hand-ordered — stdlib `PrintDefaults` alphabetizing is unacceptable for 23 flags). **Never** use `flag.ExitOnError` — it calls `os.Exit(2)`, which collides with our exit-code contract; parse errors must become exit 4.

Subcommand must be `args[0]`; flags after it. `cablecheck --verbose run` is rejected with a hint ("flags go after the subcommand") — stdlib flag stops at the first non-flag arg and fighting that is not worth it.
...
All registered on the `run` FlagSet. Sentinel defaults (0 / "") mean "mode preset decides"; help text states per-mode defaults explicitly.
...
Rule: explicit user value always wins and is then bounds-checked; an explicit value equal to the sentinel (`--tcp-duration 0`) is caught by bounds validation with a clear message. `--soak-duration`/`--soak-load` set with `mode != soak` → `ValidationError{"--soak-duration", "only valid with --mode soak"}` (fail fast, don't silently switch modes).

### 1.5 Usage text structure

Custom `Usage` per subcommand: synopsis line, then groups in fixed order — **Connection** (role/local-ip/peer-ip/interface/control-port/iperf-port/token), **Test parameters** (mode/tcp-duration/udp-duration/udp-rate/parallel-streams/soak-*/monitor-interval), **Diagnostics** (cable-test/cable-test-tdr), **Behavior** (non-interactive/no-sudo/no-report-transfer/allow-virtual-interface/verbose), **Output** (output). Footer: exit-code table and the boolean-flag caveat (`--cable-test=false`, not `--cable-test false`).
...
11. `--output`: reject if any raw path element is `..` (traversal); `filepath.Abs` + `filepath.Clean`; `os.Stat` must exist and be a directory. (Writability probe — temp-file create/delete — belongs to preflight, not config; Stat is local FS, acceptable pre-network.)

### 2.2 Token generation and distribution

PC1 with empty `--token`: 16 bytes from `crypto/rand` → `hex.EncodeToString` → 32 hex chars; `TokenGenerated = true`. **Display path is stdout via `fmt.Fprintf`, never slog** (the log file must not contain it):
...
**Parsing**: grammar `INT[.FRAC][K|M|G]`, case-insensitive suffix, decimal multipliers 1e3/1e6/1e9. Implemented with integer math, **not** `strconv.ParseFloat` (0.1-style float artifacts): parse integer part and fraction digits separately; require `len(frac) <= 9` and `10^len(frac)` divides the multiplier (so `2.5G` = 2×1e9 + 5×1e8; `1.5K` ok; `1.2345K` → error "fraction finer than unit"). Errors: empty string; zero; bare fractional without suffix (`"2.5"`); unknown suffix — with a special-case hint for `KB/MB/GB/Kb/...`: "bytes suffixes not accepted; use decimal bits: K, M, G"; negative sign; overflow past `math.MaxUint64` (checked before multiply). Bare integer = bits/sec (allowed, used in tests).

**String()**: largest suffix that renders with ≤1 decimal: `b%1e9==0 → "%dG"`, `b≥1e9 && b%1e8==0 → "%d.%dG"`, same pattern for M and K, else bare digits. Round-trips for every value CableCheck itself produces.

**UDP default-rate derivation** (in `config` since it uses run context, called after link negotiation is known):
...
- Unknown speed (ethtool unavailable / virtual interface): `rate = 100M` + warning `"link speed unknown; defaulting UDP rate to 100M — UDP loss results may be unreliable"`. This warning also lands in `Facts.UDPRateAssumed` so the evaluator soft-pedals UDP findings.
- Explicit `--udp-rate` above 95% of known negotiated speed: honored, but emits a warning and sets `Facts.UDPNearSaturation` (loss at self-inflicted saturation must not fail the cable).
...
Assembled from the (pre-evaluation) Report; every field is plain data so rules are pure and table-testable:
...
Key asymmetry, per spec: physical POOR/FAILED is **never** softened by host evidence; performance POOR **is** (→ INCONCLUSIVE) when hostLimited and physical layer is clean. Note `allPoorAreHostSensitive`: a transport POOR (e.g. ping loss) is *not* host-sensitive and keeps POOR even with hot CPUs.

### 4.5 Score (0–100)

Computed only when class ≠ INCONCLUSIVE (`Score == nil` otherwise — a number would imply confidence we don't have). Start 100, apply deductions, clamp to the class band so score and class can never contradict:
...
Bands (clamp after deductions): FAILED ≤25, POOR 26–50, WARNING 51–79, GOOD 80–94, EXCELLENT 95–100. EXCELLENT additionally requires: zero findings above Info, all critical tests ran, ping loss 0 both dirs, retrans <0.1%, CoV <15%, UDP loss <0.5%, throughput ≥90% of negotiated.

### 4.6 Recommendations

`var recommendations = map[string]string{...}` keyed by RuleID; generator walks findings in order, appends mapped strings, de-dupes preserving order; always appends the isolation-test line when class ∈ {POOR, FAILED, INCONCLUSIVE}. Entries (abridged):
...
Naming: `NN-side-tool-purpose.ext`, NN = two-digit creation sequence. Examples: `01-pc1-ethtool-link-before.txt`, `02-pc1-ip-stats-before.json`, `04-pc1-ping-stability.txt`, `06-pc1-iperf3-tcp-pc1-to-pc2.json`, `09-pc1-iperf3-udp-pc2-to-pc1.json`, `12-pc1-cablecheck-monitoring.jsonl`. Stderr of a command, when non-empty, goes to `NN-...-stderr.txt` with the same NN. The protocol log has a fixed name outside the NN scheme: `raw/cablecheck-pc1.log` (created before any command runs).

### 5.2 Duration marshaling — decision

Custom `model.Duration` marshaling to a two-field object; raw `time.Duration` is banned from `model.Report`:
...
### 5.4 Rendering — decision: strings.Builder + helpers, not text/template

23 conditional sections with computed tables, skip-notes, and per-row formatting become unmaintainable in text/template (no compile-time checking, painful conditionals, untestable in pieces). Instead:
...
One private `func sectionN(b *md, r *model.Report)` per section; `md` wraps `strings.Builder` with helpers `h2`, `kv`, `table(headers, rows)`, `note`. Absent data → section renders a one-line "Not run: <reason>" note (never silently omitted — section numbering is stable). Each section function gets its own golden test.
...
Read file (size cap 64 MiB), `json.Unmarshal` into `model.Report`, validate `schemaVersion` major == 1 (else ExitConfig with message), re-render report.md + summary.txt next to the JSON (or `--output`). **No re-evaluation** — Evaluation is part of the record. Purity of `RenderMarkdown`/`RenderSummary` (only `*model.Report` in, bytes out) is what makes this correct; the dependency test from §0 enforces it structurally.

---

## 6. `doctor` command

Flags: `--interface`, `--output` (default "."), `--no-sudo`, `--verbose`. Runs locally, no peer, read-only except a temp-file writability probe.
...
Checks, in order: (1) `ip` present + `ip -j addr` produces valid JSON; (2) `ping` present + version string; (3) `iperf3` present + version ≥3.1, capability probes (JSON always for ≥3.1; `--bidir` gated on version ≥3.7; reverse/UDP/get-server-output from `--help` scan) — missing = FAIL with distro install hints (pacman/apt/dnf); (4) `ethtool` present + version — missing = WARN (degrades to counters-limited run, not fatal for run but doctor says WARN with hint); actually FAIL per preflight requirement — decide: **FAIL**, since preflight requires it; (5) interface enumeration: each interface with state, speed/duplex if readable, IPv4 addrs, physical/virtual classification (virtual ones annotated "requires --allow-virtual-interface"); no physical candidates = WARN; (6) `sudo -n true` probe (skipped/N-A with `--no-sudo`) — failure = WARN; (7) cable-test support: version gate only (ethtool ≥5.4 + kernel note), reported as WARN "cannot verify without running — driver support varies" (never actually runs `--cable-test` from doctor); (8) output dir writability (create+remove temp file) — failure = FAIL.

Output: aligned `[PASS]/[WARN]/[FAIL] name: detail` lines + summary line `N PASS, N WARN, N FAIL`. Exit: any FAIL → `&ExitError{Code: ExitConfig}` (4); else 0. Doctor never emits exit 3.
...
`go`/`platform` from `runtime.Version()`, `runtime.GOOS/GOARCH`; protocol and schema constants from their packages. Makefile:
...
Lifecycle: `cli` creates the stderr logger immediately; `app.Run` creates the report dir right after config validation (before preflight, so preflight raw output has a home), then calls `AttachDebugFile(logger, filepath.Join(dir, "raw", "cablecheck-"+role+".log"))` and uses the tee logger for everything after; the returned Closer is deferred and closed **after** the last log write, before report-dir checksumming. Tee is a small `multiHandler` fan-out (stdlib has none); each child keeps its own level, so the file always gets Debug regardless of `--verbose`.

Token-redaction guarantees, three independent layers: (1) **construction** — protocol code logs envelopes only via `MsgAttrs` (type/id/size — never payload bytes, never the hello raw); a lint-style test greps `internal/protocol` for `Payload` appearing inside slog calls; (2) **handler** — `ReplaceAttr` on *both* handlers replaces the value of any attr keyed `token`/`payload` with `"[REDACTED]"` (ReplaceAttr fires for attrs inside groups too); (3) **types** — `config.RunConfig` implements `slog.LogValuer` with Token redacted, so even a naive `slog.Any("cfg", cfg)` is safe. Regression test: run a mock handshake, capture both sinks, assert the token substring is absent. The one legitimate token display (PC1 banner) goes to stdout via `fmt`, never through slog.
...
- Each scenario (`healthy`, `reduced-speed`, `crc-errors`, `host-limited`, `failed`) is a seed: a pre-Evaluation `model.Report` built in Go code from the same parsed-result structs the parsers produce (values traceable to `testdata/` fixtures — e.g. crc-errors seed uses the deltas from the Realtek CRC fixture). The generator runs the *real* pipeline slice in scope: `evaluate.FactsFromReport` → `evaluate.Evaluate` → set `Evaluation` → `reporting.RenderJSON/RenderMarkdown/RenderSummary` → write `examples/<name>/{report.json,report.md,summary.txt}`.
- Determinism: fixed timestamps in seeds (`2026-01-02T15:04:05Z`), fixed testId (`example-<name>`), fixed token never present (ConfigEcho has no token field), no clock, no rand.
- Drift guard: `TestExamplesUpToDate` in `internal/tools/genexamples` regenerates into `t.TempDir()` and byte-compares against `examples/`; failure message says `run: make examples`. This makes it impossible for evaluate/reporting changes to land without regenerated examples.

Expected outcomes baked into seeds: healthy → EXCELLENT/exit 0; reduced-speed → WARNING (PHY-06); crc-errors → POOR or FAILED (PHY-02 band); host-limited → INCONCLUSIVE (PERF-01 + HOST-01, clean counters); failed → FAILED (PHY-01 + PHY-03).
...
4. **`netip.ParseAddr` accepts 4-in-6 (`::ffff:192.0.2.1`) and zones** — `Unmap()` before `Is4()` or the demo docs' addresses behave inconsistently; also it rejects leading-zero octets (`192.168.001.001`) — give a clear error, users paste these.
...
7. **iperf3 JSON has no segment count** — retransmit *rate* must be estimated as `retransmits / (bytes/MSS)` with MSS from `tcp_mss_default` (fallback 1448); label it "estimated" in evidence or the 0.1% threshold looks falsely precise.
8. **TCP slow-start poisons CoV/collapse stats** — exclude the first interval from CoV/median or every run "collapses" at t=0.
9. **UDP loss when the sender never reached target rate** (CPU-bound or `--udp-rate` too high) is not cable loss — TR-07 must gate on `UDPTargetReached && CPU ≤ 90 && !UDPNearSaturation`, else host-limited runs misclassify as POOR.
...
11. **Log file inside the report dir**: create dir before preflight or early raw output has nowhere to go; flush/close the bufio'd JSON handler on interrupt *before* computing raw-file checksums, or hashes won't match transferred files.
...
14. **Counter wrap/reset**: deltas are `(after-before, ok)`; a false `ok` must remove that side's counters from Facts (`DeltaOK=false`) rather than reporting a bogus 2^32-ish delta that triggers PHY-02 FAILED.
...
16. **Go 1.24+ `GOEXPERIMENT=jsonv2`** can alter marshaling details; the drift test pins output — run CI without the experiment and note it in Makefile comments.
