...
- **Score:** 79/100
- **Reason:** Transmit-carrier/PHY error counters incremented by 3 during the test.
- **Reason:** NIC receive-ring counters moved — performance results may be limited by how quickly the host drained packets.

| Rule | Category | Severity | Finding |
| --- | --- | --- | --- |
| PHY-11 | physical | warning | Transmit-carrier/PHY error counters incremented by 3 during the test. |
| HOST-04 | host | marker | NIC receive-ring counters moved — performance results may be limited by how quickly the host drained packets. |
...
| --- | --- | --- | --- | --- |
| 2 | 938.2 Mbit/s | 938.2 Mbit/s | 941.0 Mbit/s | 0.15% |
...
| 2 | 10s | 1 | 938.5 Mbit/s | 938.2 Mbit/s | 0 | 1.20% | 935.2 Mbit/s | 940.2 Mbit/s |

## 12. TCP Throughput PC2→PC1

| Completed trials | Minimum | Lower median | Maximum | Inter-trial CoV |
| --- | --- | --- | --- | --- |
| 2 | 936.6 Mbit/s | 936.6 Mbit/s | 939.1 Mbit/s | 0.13% |

| Run | Duration | Streams | Sender | Receiver | Retransmits | Interval CoV | Min interval | Max interval |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 10s | 1 | 939.4 Mbit/s | 939.1 Mbit/s | 0 | 1.20% | 936.1 Mbit/s | 941.1 Mbit/s |
| 2 | 10s | 1 | 936.9 Mbit/s | 936.6 Mbit/s | 0 | 1.20% | 933.6 Mbit/s | 938.6 Mbit/s |
...
- **PHY-11** [physical/warning] Transmit-carrier/PHY error counters incremented by 3 during the test.
  - pc1: transmit-carrier/PHY error counters +3 during the test
- **HOST-04** [host/marker] NIC receive-ring counters moved — performance results may be limited by how quickly the host drained packets.
  - pc1: rx_fifo +2 during the test
  - pc1: rx_missed +3 during the test
